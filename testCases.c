...
//////////////////////// my test cases /////////////////////
	...
	void TestQ2_manage_students (CuTest *tc) {

		char grades_test[] =  "grades_test.txt";
		char students_test[] =  "students_test.txt";
		char output_test[] =  "output_test.txt";
		char output_golden[] = "output_golden_test.txt";
 		int size;
		int i;
		//student **list;
		//student ** list;
		//create list using the input file
		int expectedNumStudents = manage_students(grades_test,students_test,output_test );
		FILE *testfile = fopen(output_test, "r");
		FILE *goldenfile = fopen(output_golden, "r");
		int actualID,expectedID;
		double actualFinalGrade,expectedFinalGrade;

		for(i=0;i<expectedNumStudents;i++){

					fscanf(testfile,"%d",&actualID);

					fscanf(goldenfile,"%d",&expectedID);


					CuAssertIntEquals(tc, expectedID, actualID);

					fscanf(testfile,"%lf",&actualFinalGrade);
					fscanf(goldenfile,"%lf",&expectedFinalGrade);
...
//===========================================================
	CuSuite* Lab3GetSuite() {
		...
		SUITE_ADD_TEST(suite, TestQ2_manage_students);
